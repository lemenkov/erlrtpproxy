	...
	{setup,
		...
		[
			...
			{"Request basic RTP proxy functionality (ver. 20040107)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20040107\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for multiple RTP streams and MOH (ver. 20050322)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20050322\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for extra parameter in the V command (ver. 20060704)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20060704\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for RTP re-packetization (ver. 20071116)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20071116\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for forking (copying) RTP stream (ver. 20071218)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20071218\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for RTP statistics querying (ver. 20080403)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20080403\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			{"Request support for setting codecs in the update/lookup command (ver. 20081102)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20081102\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 1\n">>, Answer) end
			},
			% FIXME
			{"Request support for session timeout notifications (ver. 20081224 - not supported by this proxy)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20081224\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
			...
			{"Request support for automatic bridging (ver. 20090810 - not supported by this proxy)",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20090810\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 0\n">>, Answer) end
			},
			% FIXME should be E1
			{"Request for unsupported extensions",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 VF 20111109\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(<<"6721_89367 0\n">>, Answer) end
			},
			{"Try to create new session",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"24393_4 Uc0,8,18,101 0003e30c-c50c00f7-123e8bd9-542f2edf@192.168.0.100 192.0.43.10 27686 0003e30cc50cd69210b8c36b-0ecf0120;1\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertMatch(
							#response{
								cookie = <<"24393_4">>,
			...
			{"Try to lookup existing session",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"24393_4 Lc0,8,18,101 0003e30c-c50c00f7-123e8bd9-542f2edf@192.168.0.100 192.0.43.11 19686 0003e30cc50cd69210b8c36b-0ecf0120;1 1372466422;1\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertMatch(
							#response{
								cookie = <<"24393_4">>,
			...
			{"Request brief statistics",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 Ib\n">>),
			...
			{"Request overall statistics",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 I\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0, 1000),
						% Here is an error in rtpproxy - it should return cookie as well, e.g. prepend 6721_89367 to the beginning
						% ?assertMatch(<<"6721_89367 sessions created: ", _/binary>>, Answer) end
			...
			{"Try to close existing session",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"24393_4 D 0003e30c-c50c00f7-123e8bd9-542f2edf@192.168.0.100 0003e30cc50cd69210b8c36b-0ecf0120 1372466422\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0),
						?assertEqual(
							#response{
								cookie = <<"24393_4">>,
			...
			{"Close all active sessions",
				fun () ->
						gen_udp:send(Fd, {127,0,0,1}, ?RTPPROXY_PORT, <<"6721_89367 X\n">>),
						{ok, {Ip, Port, Answer}} = gen_udp:recv(Fd, 0, 1000),
						?assertEqual(<<"6721_89367 0\n">>, Answer) end
			}
		]
